...
(function () {
  ...
  function search() {
    ...
    searchHits.forEach(function (page) {
      const li = element('<li><a href><span></span></a></li>');

      const anchor = li.querySelector('a'),
        title = li.querySelector('a > span');

      anchor.href = `${page.href}#:~:text=${page.terms[0]}`;
      title.append(...highlight(page.title, match(page, 'title'), 0, page.title.length, input.value));
   ...
   * @param {Number} before
   * @param {Number} after
   * @param {String} [query] raw user query, caps the highlighted length to the typed characters
   * @returns {Array<Node|String>}
   */
  function highlight(text, match, before = 0, after = text.length, query = match) {
    const start = match ? text.toLowerCase().indexOf(match) : -1;
    if (start < 0) {
      return [text.slice(0, after)];
    }

    const markLen = Math.min(match.length, query.length);
    const end = start + markLen;
